...
- **Campos/perguntas:** `form_fields` tem FK para `form_templates.id`, **não** para `evaluation_forms.id`. Ou seja, hoje não existe um caminho direto "evaluation_forms → seus form_fields" no schema — a task `manage-form-fields` opera sobre `form_templates`. Antes de assumir que o fluxo onboarding-form → campos está fechado ponta a ponta, confirme com @data-engineer se falta uma coluna-ponte (ex: `evaluation_forms.form_template_id`).
